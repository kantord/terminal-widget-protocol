...
### 1.1 Graphics exist, but as opaque pixels bolted on the side

Sixel, the Kitty graphics protocol, and iTerm2 inline images already put bitmaps
on the screen. But they place **finished, opaque pixels**: the sender renders
everything externally and hands the terminal a blob to position. The terminal
has no idea what's *in* it. That blob does not participate in anything the
terminal does — it doesn't reflow, doesn't theme, its text can't be selected or
read by a screen reader, and it must be regenerated by hand whenever the font or
window changes. The gap is not "more pixels"; it is a way to render rich content
**through the terminal's own pipeline** rather than around it.

### 1.2 The terminal already solves the hard parts — external renderers must re-solve them, and drift

An application that renders its own pixels is forced to reproduce, externally and
imperfectly, everything the terminal already does:

- **Font rendering** — the single hardest part. The terminal already rasterizes
  glyphs with the user's exact font, size, hinting, and metrics. Matching that
  from outside (so widget text looks like terminal text) is a perennial,
  never-quite-right struggle.
- **The layout grid.** Aligning external graphics to the terminal's cell grid —
  which depends on the live, per-terminal cell pixel size — is fiddly and breaks
  on font/DPI changes (the problem TWP's cell units address; §7.3).
- **The color theme.** Making a rendered widget match the user's terminal color
  scheme requires discovering and tracking that palette; most external renderers
  simply don't, and look foreign as a result (the problem `term(...)` addresses;
  §8).
- **Terminal features.** Selection, copy, search, reflow, and accessibility all
  operate on the terminal's model of the screen. An opaque image is invisible to
  every one of them.

A terminal that renders a TWP scene gets all of this *for free*, because it owns
the font stack, the grid, the palette, and the screen model. The same widget
text can be real, selectable, screen-reader-visible text; the same colors are
*the user's* colors. (The bundled polyfill, being external, deliberately accepts
the drift this section describes — which is exactly why native integration is the
intended implementation; §3.3.)
